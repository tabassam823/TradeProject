"""
OpenBB Sentiment-Filtered Trend Strategy.
Combines fast/slow EMA trend crossovers with news/market sentiment filter.
"""
...
class SentimentFilteredTrendStrategy(BaseStrategy):
    ...
    def generate_signal(self, ohlcv_df: pd.DataFrame, market_state: Optional[Dict[str, Any]] = None) -> float:
        ...
        close = ohlcv_df['close'].tail(self.slow_period * 2)
        ...
        if market_state and "macro_factors" in market_state:
            fng = market_state["macro_factors"].get("CRYPTO_FEAR_GREED", 50.0)
            sentiment_score = (fng - 50.0) / 50.0
        else:
        ...
        if fast_ema > slow_ema and sentiment_score >= self.min_sentiment:
            return 3.0
